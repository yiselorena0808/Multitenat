...
export const confirmed = {
    compile: wrapCompile(RULE_NAME, ['string', 'boolean', 'number', 'enum'], (args) => {
        const res = {
            compiledOptions: {
    ...
    validate(value, { confirmationFieldName }, { errorReporter, field, pointer, arrayExpressionPointer, root, tip }) {
        ...
        if (getFieldValue(confirmationFieldName, root, tip) != value) {
            errorReporter.report(resolveAbsoluteName(confirmationFieldName, pointer), RULE_NAME, DEFAULT_MESSAGE, arrayExpressionPointer);
        }
    },
};
