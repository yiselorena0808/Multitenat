...
class ActividadesLudicasController {
  ...
  async listarIdActividad({ params, response, request }: HttpContext) {
    try {
      ...
      const actividad = await this.service.listarId(id, empresaId);
  ...
  async eliminarActividad({ params, response, request }: HttpContext) {
    try {
      const id = params.id;
      const empresaId = (request as any).empresaId;
      const resultado = await this.service.eliminar(id, empresaId);
  ...
  async actualizarActividad({ request, response, params }: HttpContext) {
    try {
      ...
      const actividad = await this.service.actualizar(id, empresaId, datos);
